...
from libqtile.widget import ThermalSensor, Memory, CPU
...
keys = [
    ...
    # Key([], "XF86AudioLowerVolume", lazy.spawn("amixer -c -0 sset Master 1+ unmute")),
    Key([mod], "F11", lazy.spawn("scrot /tmp/screenshot.png -e 'convert $f png:- | xclip -selection clipboard -t image/png -i; rm $f'")),
    ...
    Key([mod, "control"], "5", lazy.window.togroup("5")),
...
layouts = [
    ...
    layout.Columns(border_focus="#928374", border_normal="#00000000", border_width=1, margin=1),
...
colors = [
    ["#1d2021", "#1d2021"], # panel background
    ...
    ["#ebdbb2", "#ebdbb2"], # group names
    ["#fbf1c7", "#fbf1c7"], # even widgets
    ["#7c6f64", "#7c6f64"], # inactive screens
    ["#83a598", "#83a598"], # active window numbers
    ["#d5c4a1", "#d5c4a1"], # frame around the numbers
    ["#8ec07c", "#8ec07c"]  # annotation text
]

class ColoredThermalSensor(ThermalSensor):
    thresholds = {
        "critical": 75,
        "warning": 55,
    }
    ...
    def poll(self):
        temp_str = super().poll()
        temp = float(temp_str.replace("°C", ""))  # Remove the unit and convert to float
        if temp >= self.thresholds["critical"]:
            self.foreground = "#fb4934"
        elif temp >= self.thresholds["warning"]:
            self.foreground = "#fabd2f"
        else:
            self.foreground = "#ebdbb2"
        return temp_str  # Return the original string with the unit
...
class ColoredMemory(Memory):
    ...
    def poll(self):
        ...
        if percent >= self.thresholds["critical"]:
            ...
            self.foreground = "#ebdbb2"
        used = sizeof_fmt(mem.used)
        return used

class ColoredCPU(CPU):
    thresholds = {
        "critical": 75,
        "warning": 55,
    }
